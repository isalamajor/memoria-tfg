...
Para la gestión de imágenes de perfil, se ha implementado la librería \textbf{Multer}, que permite procesar la subida de archivos de forma eficiente, integrándose directamente con el sistema de archivos del servidor.

La configuración del sistema de subida de archivos se divide en tres pilares fundamentales:

\begin{enumerate}
    \item Almacenamiento Persistente: Se utiliza el motor de almacenamiento en disco para definir la ruta de destino. Una decisión de diseño clave ha sido el renombrado de archivos: en lugar de conservar el nombre original (que podría contener caracteres inválidos o colisionar con otros usuarios), el archivo se renombra utilizando el identificador del usuario extraído del token JWT. Esto garantiza que cada usuario solo posea una imagen activa, sobrescribiendo la anterior automáticamente en caso de actualización.

    \item Filtrado de Seguridad: Para prevenir la subida de archivos maliciosos (como scripts ejecutables disfrazados), se ha implementado un filtro por \textit{mimetype} y extensión. El sistema solo acepta formatos de imagen estándar (\texttt{JPEG, JPG, PNG}), verificando tanto la extensión del archivo como su tipo MIME real para asegurar la integridad del recurso.

    \item Control de Recursos: Se establece una restricción en el tamaño del archivo (fijando un máximo de 5 MB) para evitar \textbf{ataques de denegación de servicio (DoS)} por agotamiento de almacenamiento o ancho de banda, garantizando que el servidor procese únicamente archivos de dimensiones razonables para una imagen de perfil.
\end{enumerate}

Este módulo se exporta como un middleware independiente, lo que permite inyectarlo exclusivamente en las rutas de la API que requieren capacidad de subida, manteniendo el principio de responsabilidad única en el código.
...
Otra forma de optimización clave utilizada en el desarrollo del backend es el uso del Aggregate o Framework de Agregación de MongoDB. Esta herramienta permite acceder a diferentes colecciones y aplicar filtrados, ordenamientos, uniones, proyecciones y paginación en una única operación atómica. Esto evita el procesar grandes volúmenes de datos en la capa de JavaScript o caer en el problema de las \textbf{N+1 consultas} (realizar múltiples peticiones a la base de datos para completar un solo registro). Así, delegamos la carga de procesamiento al motor de la base de datos, manteniendo tiempos de respuesta bajos y escalables.
